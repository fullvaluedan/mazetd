...
const dir = path.join(root, 'assets/staging/enemy-family/wisp');
let fails = 0;
const check = (n, ok) => { console.log(ok ? '  ok  ' : '  FAIL', n); if (!ok) fails++; };
const staticSprite = inspectPng(path.join(dir, 'wisp-static-64-grid-v1.png'));
...
for (const state of ['hover', 'defeat']) {
  const sheet = inspectPng(path.join(dir, `wisp-${state}-2x2-grid-v1.png`));
  const frames = inspectAtlasFrames(path.join(dir, `wisp-${state}-2x2-grid-v1.png`), [2, 2]);
...
check('map proof is 12x16 grid scale', inspectPng(path.join(dir, 'wisp-map-proof-v1.png')).width === 768);
console.log(fails ? `WISP_STAGING_QC_FAIL (${fails})` : 'WISP_STAGING_QC_OK');
