...
const dir = path.join(root, 'assets/staging/enemy-family/warden');
let fails = 0;
const check = (n, ok) => { console.log(ok ? '  ok  ' : '  FAIL', n); if (!ok) fails++; };
const staticSprite = inspectPng(path.join(dir, 'warden-static-64-grid-v1.png'));
...
for (const state of ['walk', 'defeat']) {
  const sheet = inspectPng(path.join(dir, `warden-${state}-2x2-grid-v1.png`));
  const frames = inspectAtlasFrames(path.join(dir, `warden-${state}-2x2-grid-v1.png`), [2, 2]);
...
check('map proof is 12x16 grid scale', inspectPng(path.join(dir, 'warden-map-proof-v1.png')).width === 768);
console.log(fails ? `WARDEN_STAGING_QC_FAIL (${fails})` : 'WARDEN_STAGING_QC_OK');
