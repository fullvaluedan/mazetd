// Sniper promotion checks the rotated static order and approved atlas geometry.
...
const sniperRoot = path.join(root, 'assets/staging/tower-family/sniper');
let fails = 0;
...
console.log('Sniper family: five rotated full-footprint 2x2 static tiers');
for (let level = 1; level <= 5; level++) {
  const png = inspectPng(path.join(sniperRoot, `sniper-lv${level}-2x2-grid-v4.png`));
...
console.log('Sniper attack atlas: four distinct full-footprint 2x2 frames');
const attackPath = path.join(sniperRoot, 'sniper-attack-2x2-grid-v3.png');
...
console.log(fails === 0 ? 'SNIPER_STAGING_QC_OK' : `SNIPER_STAGING_QC_FAIL (${fails})`);
