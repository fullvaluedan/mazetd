...
console.log('Tower asset manifest: live roster gains level variants and attack sheets:');
{
  ...
  check('cannon lv1 manifest entry exists', !!cannonLv1 && cannonLv1.out === 'towers/cannon-lv1.png');
  check('cannon lv5 manifest entry exists', !!cannonLv5 && cannonLv5.out === 'towers/cannon-lv5.png');
  check('cannon attack sheet entry exists', !!cannonAtk && cannonAtk.out === 'sheets/tower-cannon-attack.png' && cannonAtk.frames === 4);
  check('gold level variants are listed', !!goldLv5 && goldLv5.out === 'towers/gold-lv5.png');
  ...
  check('tower levels inherit sprite metadata', arrowLv5?.meta?.kind === 'sprite' && arrowLv5.meta.logical?.[0] === 32);
