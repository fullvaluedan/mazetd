...
const { Viewport, HUD_TOP_GUTTER, HUD_BOTTOM_GUTTER } = await import('../src/ui/viewport.js');
...
{
  const stageW = 1280, stageH = 800;
  const cssH = stageH - HUD_TOP_GUTTER - HUD_BOTTOM_GUTTER;
  const cssW = Math.floor(CANVAS_W * cssH / CANVAS_H);
  const c = fakeCanvas(cssW, cssH);
  const v = new Viewport(c, { style: {} }, box(1280, 800));
  check('scale = (800 - gutters) / 576', Math.abs(v.scale - cssH / CANVAS_H) < 1e-9, v.scale.toFixed(4));
  check('cssH fits the playfield gutter gap', v.cssH === cssH);
  check('cssW letterboxed', v.cssW === cssW, `${v.cssW}`);
  check('centered with reserved gutters', v.left === Math.floor((stageW - v.cssW) / 2) && v.top === HUD_TOP_GUTTER);
  ...
  check('worldToUi hits the css corner', Math.abs(u.x - (v.left + v.cssW)) < 1e-9 && Math.abs(u.y - (v.top + v.cssH)) < 1e-9);

  // client -> world via the live rect (rect 1244x800 at 10,20)
  const w = v.clientToWorld(10 + cssW, 20 + cssH);
...
console.log('DPR 2 backing store (capped at 2):');
{
  ...
  const stageW = 896, stageH = 576;
  const cssH = stageH - HUD_TOP_GUTTER - HUD_BOTTOM_GUTTER;
  const cssW = Math.floor(CANVAS_W * cssH / CANVAS_H);
  const c = fakeCanvas(cssW, cssH);
  const v = new Viewport(c, null, box(stageW, stageH));
  check('scale respects the HUD gutters', Math.abs(v.scale - cssH / CANVAS_H) < 1e-9);
  check('backing store = css * 2 (capped)', c.width === cssW * 2 && c.height === cssH * 2);
  check('k = backing store / world width', Math.abs(v.k - c.width / CANVAS_W) < 1e-9);
...
{
  const cssH = 576 - HUD_TOP_GUTTER - HUD_BOTTOM_GUTTER;
  const cssW = Math.floor(CANVAS_W * cssH / CANVAS_H);
  // rect matches the canvas css box after the reserved HUD gutters.
  const v = new Viewport(fakeCanvas(cssW, cssH), { style: {} }, box(896, 576));
  for (const z of [1.0, 1.7, 2.5]) {
    ...
    let ok = true;
    for (const [cx, cy] of [[10, 20], [10 + cssW, 20 + cssH], [10 + 123, 20 + 300]]) {
      const w = v.clientToWorld(cx, cy);
      const u = v.worldToUi(w.x, w.y);
      // #ui spans the stage, so subtract the inset board offset to compare
      if (Math.abs(u.x - v.left - (cx - 10)) > 1e-9 || Math.abs(u.y - v.top - (cy - 20)) > 1e-9) ok = false;
    }
...
console.log('Camera: min zoom on a 12x16 board = the pre-camera letterbox exactly:');
{
  ...
  const preScale = Math.min(vw / (12 * 32), (vh - HUD_TOP_GUTTER - HUD_BOTTOM_GUTTER) / (16 * 32));
  ...
  check('centered like pre-camera',
    v.left === Math.floor((vw - v.cssW) / 2) && v.top === HUD_TOP_GUTTER + Math.floor(((vh - HUD_TOP_GUTTER - HUD_BOTTOM_GUTTER) - v.cssH) / 2));
  let got = null;
  v.applyTransform({ setTransform: (...a) => { got = a; } });
...
{
  const cssH = 800 - HUD_TOP_GUTTER - HUD_BOTTOM_GUTTER;
  const cssW = Math.floor(CANVAS_W * cssH / CANVAS_H);
  const v = new Viewport(fakeCanvas(cssW, cssH), { style: {} }, box(1280, 800));
  ...
  check('worldToUi = (w - cam) * scale * zoom',
    Math.abs(u.x - (v.left + (500 - 200) * (v.cssW / CANVAS_W) * 2)) < 1e-9 &&
    Math.abs(u.y - (v.top + (350 - 150) * (v.cssH / CANVAS_H) * 2)) < 1e-9,
...
console.log('Gestures: sub-slop tap clicks; a drag pans (select mode off) and suppresses the click:');
{
  ...
  const cssH = stageH - HUD_TOP_GUTTER - HUD_BOTTOM_GUTTER;
  const cssW = Math.floor(CANVAS_W * cssH / CANVAS_H);
  const c = gestureCanvas(cssW, cssH);
  const v = new Viewport(c, { style: {} }, box(stageW, stageH));
  let clicks = 0;
  ...
  // finger left 40 / up 20 css px at the live fit scale and zoom 2
  const expectedDx = 40 / ((v.cssW / CANVAS_W) * v.zoom);
  const expectedDy = 20 / ((v.cssH / CANVAS_H) * v.zoom);
  check('drag pans: css delta / (scale*zoom)',
    Math.abs(v.camX - (224 + expectedDx)) < 1e-9 && Math.abs(v.camY - (144 + expectedDy)) < 1e-9,
    `cam ${v.camX},${v.camY}`);
...
console.log('Gestures: pinch through the real handlers keeps the midpoint world point pinned:');
{
  ...
  check('midpoint world point stays under the midpoint',
    Math.abs(u.x - v.left - 448) < 1e-6 && Math.abs(u.y - v.top - 288) < 1e-6, `${u.x.toFixed(2)},${u.y.toFixed(2)}`);
  ...
  check('midpoint pan drags the world point along',
    Math.abs(v.zoom - 1.6) < 1e-6 && Math.abs(u.x - v.left - 498) < 1e-6 && Math.abs(u.y - v.top - 288) < 1e-6,
    `${u.x.toFixed(2)},${u.y.toFixed(2)}`);
