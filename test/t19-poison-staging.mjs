// Poison promotion requires source geometry checks beyond manifest metadata.
...
const poisonRoot = path.join(root, 'assets/staging/tower-family/poison');
let fails = 0;
...
console.log('Poison family: five full-footprint 2x2 static tiers');
for (let level = 1; level <= 5; level++) {
  const png = inspectPng(path.join(poisonRoot, `poison-lv${level}-2x2-grid-v3.png`));
...
console.log('Poison attack atlas: four distinct full-footprint 2x2 frames');
const attackPath = path.join(poisonRoot, 'poison-attack-2x2-grid-v3.png');
...
console.log(fails === 0 ? 'POISON_STAGING_QC_OK' : `POISON_STAGING_QC_FAIL (${fails})`);
